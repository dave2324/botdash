...
router.get('/me/transactions', async (req, res) => {
  try {
    ...
    // Fetch transaction logs for the user with status information
    const result = await pool.query(
      `SELECT 
        tl.id,
        tl.transaction_type,
        ...
        tl.metadata,
        CASE 
          WHEN tl.transaction_type = 'deposit' THEN d.status
          WHEN tl.transaction_type = 'withdrawal' THEN w.status
          WHEN tl.transaction_type = 'refund' THEN 'completed'
          ELSE 'completed'
        END as status
      FROM transaction_logs tl
      LEFT JOIN deposits d ON tl.transaction_type = 'deposit' AND tl.transaction_id = d.transaction_id
      LEFT JOIN withdrawals w ON tl.transaction_type = 'withdrawal' AND tl.transaction_id = w.transaction_id
      WHERE tl.user_id = $1 
